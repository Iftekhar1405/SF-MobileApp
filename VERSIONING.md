...
Only `master` creates mobile app releases.
...
The mobile app shows the version at the bottom center of the profile/menu drawer as `vX.X.X`.
...
3. Update `package.json`, `package-lock.json`, `app.json`, and `CHANGELOG.md`.
...
`semantic-release` updates `package.json` and `package-lock.json`. This repo also runs `scripts/sync-expo-version.cjs` during the release prepare step so Expo's `app.json` version stays in sync with the same release number.
...
fix: correct cart total display
docs: update release notes
refactor: simplify profile drawer
```
